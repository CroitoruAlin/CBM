...
import math
def build_optimizer_wresnet(model):
    return optim.SGD(model.parameters(), lr=0.1,
                               weight_decay=5e-4,
                               momentum=0.9)

def build_optimizer_wresnet_other(model):
    return optim.SGD(model.parameters(), lr=0.1,
                               weight_decay=1e-4,
                               momentum=0.9)
...
def train_food101(args):
    ...
    args.num_epochs = 200
    args.model_name = 'wr_food101b'
    curriculum = lin_repeat_food()
    args.percent = curriculum * 40
    #args.percent = baseline(200)
    print(args.percent)
    print(len(args.percent))
    wresnet = Wide_ResNet(52,2,0.3,args.num_classes,4)
    ...
    trainer = Trainer(wresnet, train_loader, test_loader, args,build_optimizer_wresnet_other)
    trainer.train()
...
def train_cifar10(args):
    ...
    args.testlo=test_loader
    trainer = Trainer(wresnet, train_loader, test_loader, args,build_optimizer_wresnet)
    trainer.train()
...
def train_tinyimagenet(args):
    ...
    trainer = Trainer(wresnet, train_loader, val_loader, args,build_optimizer_wresnet_other)
    trainer.train()
